# Hi there, I'm Joel Jacob 

## 🎓 About Me
Final year **Electronics & Biomedical Engineering** student at Govt. Model Engineering College, Kochi, passionate about **medical device design** and **embedded systems**. Currently working as a Project Intern at **Ubio Biotechnology Systems**, developing a semi-automated real-time PCR machine.

## 🔬 Current Focus
- 🧬 Developing hardware for molecular diagnostics (PCR Machine)
- 💡 PCB design for medical-grade instrumentation
- 🔧 Embedded systems programming with STM32
- 📊 Biomedical signal processing and analysis

## 🛠️ Technical Skills

### PCB Design & Hardware
![KiCAD](https://img.shields.io/badge/KiCAD-314CB0?style=for-the-badge&logo=kicad&logoColor=white)
![AutoCAD](https://img.shields.io/badge/AutoCAD-EE3124?style=for-the-badge&logo=autodesk&logoColor=white)
![Proteus](https://img.shields.io/badge/Proteus-00599C?style=for-the-badge&logo=proteus&logoColor=white)

### Programming
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=for-the-badge&logo=arduino&logoColor=white)

### Microcontrollers & Embedded
- STM32 Microcontroller Family
- Arduino Platform
- Embedded C Programming
- Motor Control Systems
- Thermal Management (Peltier)
- ADC Integration (24-bit)

### Signal Processing
- ECG Signal Analysis
- Butterworth Filtering
- Data Acquisition Systems
- Real-time Processing

## 🚀 Featured Projects

### 🧬 Semi-Automated Real-Time PCR Machine
**Ubio Biotechnology Systems | Ongoing**

Developing a molecular diagnostics platform with focus on hardware design:
- Multi-layer PCB design for motor drivers, thermal control, and optical systems
- 24-bit ADC integration for fluorescence detection
- STM32-based embedded control system
- **Tech:** KiCAD, STM32, Peltier Modules, Stepper Motors, Optical Systems



---

### ❤️ Heart Rate Variability Monitoring System
**Academic Project | 3 Months**

Clinical-grade ECG acquisition and HRV analysis system:
- AD8232-based ECG front-end circuit design
- Butterworth filtering for noise reduction
- Real-time R-peak detection and RR-interval analysis
- Achieved clinical accuracy in HRV measurements
- **Tech:** Python, Arduino, AD8232, Signal Processing



---

### 🌤️ Weather Forecasting with Neural Networks
**Academic Project | 2 Weeks**

Machine learning model for weather prediction:
- Multi-layer ANN architecture
- Time-series forecasting
- Historical weather data analysis
- **Tech:** Python, TensorFlow/Keras, Data Science



## 📚 Certifications
- 🚀 Rocket Propulsion & Spacecraft Dynamics - Kodacy (ISRO)
- 🐍 Python Programming Bootcamp - LetsUpgrade (NSDC)
- ⚡ C++ Programming Bootcamp - LetsUpgrade (NSDC)
- 📊 Data Fundamentals - IBM
- ⚙️ Industrial Training - KELTRON

## 📫 Let's Connect!
- 📧 Email: joeljacob1254@gmail.com
- 💼 LinkedIn: [linkedin.com/in/joeljacob](https://www.linkedin.com/in/joeljacob1254/)
- 📱 Phone: +91 9846396477

## 📊 GitHub Stats
![Joel's GitHub stats](https://github-readme-stats.vercel.app/api?username=yourusername&show_icons=true&theme=tokyonight)

## 🌱 Currently Learning
- Advanced PCB design techniques
- Medical device standards (ISO 13485, IEC 60601)
- Deep learning for medical imaging
- FPGA-based signal processing

---

💡 *Open to opportunities in biomedical engineering, medical device R&D, and embedded systems development!*
